...
class ImageConverter(object):
	...
	def load_img(self, path):
		...
		image = self.max_act*(1 - abs(image/np.max(image))) # normalization
...
def cmd_in():
	...
	while True:
		...
		if (cmd == "stop"):
			...
			try:
				ImageConverter.max_act = int(cmd_param)*4
				ImageConverter.SendIMG("img.bmp")
			except:
				print("Something is not right with the command.\n")
